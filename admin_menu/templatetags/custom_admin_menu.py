...
def make_menu_group(title, children=None, weight=None):
    return MenuGroup(
        title=title, children=children, weight=weight or get_group_weight(title)
    )
    ...
    for app in apps:
        ...
        for model in app["models"]:
            ...
            for item in extra:
                ...
                submenu = make_menu_item(
                    url=url, title=capfirst(extra_title), weight=weight
                )
                group.children.append(submenu)
...
def get_license_status():
    data = T01Cfg10.objects.all().first()
    status = ""
    if data:
        today = date.today()
        if today > data.date_expiry:
